...
/*   Updated: 2023/08/10 15:55:20 by amehrotr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_exit(t_win *win)
{
	...
	else if (win->mapstr->exit_pos == 13)
		win->img->exit = mlx_xpm_file_to_image(win->mlx_ptr, \
		"textures/Exit/Exit_RL.xpm", &win->img->w, &win->img->h);
	else if (win->mapstr->exit_pos == 14)
		win->img->exit = mlx_xpm_file_to_image(win->mlx_ptr, \
		"textures/Exit/Exit_UD.xpm", &win->img->w, &win->img->h);
	else
		init_exit1(win);
}
