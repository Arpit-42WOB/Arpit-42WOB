...
/*   Updated: 2023/08/10 15:52:12 by amehrotr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	game_window(t_win *win)
{
	...
	win->win_ptr = mlx_new_window(win->mlx_ptr, win->mapstr->w * TILE_SIZE, \
		win->mapstr->h * TILE_SIZE + 50, "So_Long_Bonus");
	win->moves = 0;
	win->over = 0;
...
void	display_image(t_win *win, int y, int x)
{
	while (win->map[++y])
	{
		...
		while (win->map[y][++x])
		{
			...
			else if (win->map[y][x] == ENEMY)
				mlx_put_image_to_window(win->mlx_ptr, win->win_ptr, \
					win->img->enemy, x * TILE_SIZE, y * TILE_SIZE);
