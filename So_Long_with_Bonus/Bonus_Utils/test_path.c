...
/*   Updated: 2023/08/09 18:11:40 by amehrotr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_move(char **map_dup, int x, int y, t_win *win)
{
	if (map_dup[y][x] == 'C')
		win->to_collect += 1;
	if (map_dup[y][x] == 'E' || map_dup[y][x] == 'H')
	{
		if (map_dup[y][x] == 'E')
			win->to_exit += 1;
		map_dup[y][x] = '1';
	...
	map_dup[y][x] = 'F';
	if (map_dup[y][x + 1] != '1' && map_dup[y][x + 1] != 'F' \
			&& map_dup[y][x + 1])
		check_move(map_dup, x + 1, y, win);
	if (map_dup[y][x - 1] != '1' && map_dup[y][x - 1] != 'F' \
			&& map_dup[y][x - 1])
		check_move(map_dup, x - 1, y, win);
	if (map_dup[y + 1][x] != '1' && map_dup[y + 1][x] != 'F' \
			&& map_dup[y + 1][x])
...
void	calculate_moves(t_win *win, int x, int y)
{
	...
	mlx_string_put(win->mlx_ptr, win->win_ptr, ((x * 64) / 2) - 50, \
					y * 64 + 20, 0xFFFFFF, win->put_moves);
	mlx_string_put(win->mlx_ptr, win->win_ptr, ((x * 64) / 2) - 58, \
					y * 64 + 40, 0xFFFFFF, win->put_collect);
