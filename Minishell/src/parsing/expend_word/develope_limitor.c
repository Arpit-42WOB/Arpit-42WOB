...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/27 13:20:40 by amehrotr          #+#    #+#             */
/*   Updated: 2023/10/27 13:20:41 by amehrotr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*develope_limitor(t_token *stc, char *str, int i)
{
	...
	while (str && str[i])
	{
		...
		if (prec != quote)
			prec = update_quote_success(stc, &i, quote, &expand_str);
		else
		{
...
int	limitor(t_token *stc, char *str)
{
	...
	new = NULL;
	new = develope_limitor(stc, str, 0);
	if (str)
		free(str);
	if (new == NULL)
		return (50);
